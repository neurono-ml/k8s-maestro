use k8s_maestro::{clients::MaestroK8sClient, entities::{builders::BuildJob, container::{EnvironmentVariableFromObject, MaestroContainer}, job::{WorkflowStepBuilder, WorkflowNameType}}};
...
pub async fn main() -> anyhow::Result<()> {
    ...
    let list_job = maestro_client.create_job(&test_job_input).await?;
    list_job.wait().await?;
    list_job.delete_associated_pods().await?;
    list_job.delete().await?;
...
pub fn build_job(image: &str, generate_name: &str, namespace: &str, backoff_limit: usize,
    ...
    let job = WorkflowStepBuilder::new(&job_name, namespace)
